...
def train(env, agent, cfg):
    ...
    writer = SummaryWriter(
        f'./exp/{cfg.algo_name}_{cfg.env_name.replace("/", "-")}')
            ...
            if not use_rnn:
                ...
                for key, value in monitors.items():
                    if not np.isnan(value):
                        writer.add_scalar(
                            f'train/{key}', value, global_step=agent.learn_step)
                writer.add_scalar('train/lr', agent.lr,
                                  global_step=agent.learn_step)
            if done:
                break
        if use_rnn:
            monitors = agent.update()
