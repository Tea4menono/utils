...
class NoisyLinear(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            weight = self.weight_mu + \
                self.weight_sigma.mul(self.weight_epsilon)  # mul是对应元素相乘
    ...
    def reset_parameters(self):
        ...
        self.bias_sigma.data.fill_(
            self.sigma_init / math.sqrt(self.out_features))  # 这里要除以out_features
    ...
    def __init__(self,
        ...
        for i, (in_channels, out_channels) in enumerate(channels):
            ...
            self.conv_layers.add_module(
                f'bn_{i}', nn.BatchNorm2d(out_channels))
            self.conv_layers.add_module(f'relu_{i}', nn.ReLU())
            self.conv_layers.add_module(
                f'pool_{i}', nn.MaxPool2d(kernel_size=(2, 2)))
...
class MultiHeadAttention(nn.Module):
    def __init__(self,
        ...
        if last_act:
            self.out = MLP([n_heads * self.d_v, self.out_dim],
                           act, last_act=True)
        else:
    ...
    def forward(self, q, k, v, mask=None):
        batch_size = q.size(0)
        q = self.Q(q).view(batch_size, -1, self.n_heads,
                           self.d_k).transpose(1, 2)
        k = self.K(k).view(batch_size, -1, self.n_heads,
                           self.d_k).transpose(1, 2)
        v = self.V(v).view(batch_size, -1, self.n_heads,
...
class PSCN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc2 = MLP(
            [self.hidden_dim // 2, self.hidden_dim // 2], last_act=True)
        self.fc3 = MLP(
            [self.hidden_dim // 4, self.hidden_dim // 4], last_act=True)
        self.fc4 = MLP(
            [self.hidden_dim // 8, self.hidden_dim // 8], last_act=True)
