...
class SAC:
    def __init__(self, cfg):
        ...
        self.log_alpha = torch.tensor(
            np.log(0.01), requires_grad=True, device=cfg.device, dtype=torch.float32)
    ...
    def update(self):
        ...
        action, reward, done = action.view(-1,
                                           21), reward.view(-1, 1), done.view(-1, 1)
        ...
        q = torch.min(self.critic1(state, new_action),
        ...
        alpha_loss = torch.mean(self.log_alpha *
                                (-log_prob - self.cfg.target_entropy).detach())
...
if __name__ == '__main__':
    ...
    plt.title('Traning Total Reward per Episode(SAC)')
    plt.xlabel('Episode')
